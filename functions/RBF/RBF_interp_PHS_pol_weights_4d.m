function w = RBF_interp_PHS_pol_weights_4d (X0,XE0,m,d)
...
x = X0(:,1); y = X0(:,2); z = X0(:,3); x4 = X0(:,4); 
xe = XE0(:,1); ye = XE0(:,2); ze = XE0(:,3); xe4 = XE0(:,4); 

x = x-xe; y = y-ye;   z = z-ze; x4 = x4-xe4; % Shift nodes so stencil centered at origin

n = length(x);

% scale the distance to O(1)

scale = sqrt(x(end)^2+y(end)^2 +z(end)^2+x4(end)^2); 

% x = x/scale; y = y/scale;   z = z/scale;


r =  hypot(hypot(hypot(x,y),z),x4);

% ------ RBF part --------------------------------------------------------

A0 = hypot(hypot(hypot(bsxfun(@minus,x,x'),bsxfun(@minus,y,y')), bsxfun(@minus,z,z')),  bsxfun(@minus,x4,x4')).^m; % RBF matrix
...
if d == -1 % Special case; no polynomial terms,
    ...
    X4 = x4(:,ones(1,d+1)); X4(:,1) = 1; X4 = cumprod( X4,2);

    np = (d+1)*(d+2)*(d+3)*(d+4)/6/4; % Number of polynomial terms
    XY = zeros(n,np);
        ...
        for k1 = k:-1:0
            ...
            for k2 =    k-k1:-1:0
                ...
                for k3 = k-k1-k2:-1:0
                    ...
                    XY(:,col) = X(:,k1+1).*Y(:,k2+1).*Z(:,k3+1).*X4(:,k4+1);
